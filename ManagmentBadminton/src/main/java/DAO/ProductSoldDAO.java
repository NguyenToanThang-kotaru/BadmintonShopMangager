...
public class ProductSoldDAO {
    ...
    public List<ProductSoldDTO> getByDetailSaleInvoiceID(String salesInvoiceDetailID) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                ProductSoldDTO.setProductId(rs.getString("ProductID"));
    ...
    // Get ProductSoldDTO by ProductID and DetailSaleInvoiceID
    public List<ProductSoldDTO> getByProductIDAndDetailSaleInvoiceID(String productId, String detailSaleInvoiceID) {
        List<ProductSoldDTO> productSoldList = new ArrayList<>();
        String query = "SELECT * FROM product_sold WHERE ProductID = ? AND SalesInvoiceDetail = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, productId);
            stmt.setString(2, detailSaleInvoiceID);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                ProductSoldDTO ProductSoldDTO = new ProductSoldDTO();
                ProductSoldDTO.setDetailSaleInvoiceID(rs.getString("SalesInvoiceDetail"));
    ...
    public boolean add(ProductSoldDTO ProductSoldDTO) {
        String query = "INSERT INTO product_sold (SalesInvoiceDetail, ProductID, Series) VALUES (?, ?, ?)";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, ProductSoldDTO.getDetailSaleInvoiceID());
            stmt.setString(2, ProductSoldDTO.getProductId());
            stmt.setString(3, ProductSoldDTO.getSeries());
